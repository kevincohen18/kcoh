...
/**
 * Client child of `app/about/page.tsx`. The page file stays a server
 * component so it can keep its (English) `metadata` export; all
 * locale-aware JSX lives here. See `web/lib/i18n/PATTERN.md`, "Making a
 * component locale-reactive — checklist", step 5.
 */
export function AboutContent() {
  ...
  const intro = aboutIntro[locale];
  const story = founderStory[locale];
  const proof = proofPoints[locale];
  const details = stepDetails[locale];
  const princ = principles[locale];
  ...
  return (
    <>
      ...
      <section className="border-t border-border bg-bg">
        <Container className="grid gap-12 py-20 md:py-28 lg:grid-cols-12">
          ...
          <Reveal delay={0.1} className="lg:col-span-5">
            ...
            <p className="text-center text-sm text-fg-subtle">{f.role}</p>
          </Reveal>
        </Container>
      </section>
      ...
      <CTASection heading={copy.cta.heading} subline={copy.cta.subline} />
    </>
  );
}
